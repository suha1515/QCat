...
void main()
{
	...
	Output.FragPos = vec3(u_Transform * totalPosition);
...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient = light.ambient * vec3(texture(ambientMap, Input.TexCoords));
    vec3 diffuse = light.diffuse * diff * vec3(texture(albedoMap, Input.TexCoords));
