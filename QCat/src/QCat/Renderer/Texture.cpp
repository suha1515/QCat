...
namespace  QCat
{
	...
	Ref<Texture2D> Texture2D::Create(const std::string& path,bool gamacorrection)
	{
		switch (Renderer::GetAPI())
		{
		case RenderAPI::API::None:			QCAT_CORE_ASSERT(false, "RenderAPI isnt selected! : VertexArray Error"); return nullptr;
		case RenderAPI::API::OpenGL:		return CreateRef<OpenGLTexture2D>(path,gamacorrection);
		case RenderAPI::API::DirectX11:		return CreateRef<DX11Texture2D>(path,gamacorrection);
		}
		QCAT_CORE_ASSERT(false, "Unknown RenderAPI!");
		return nullptr;
	}
